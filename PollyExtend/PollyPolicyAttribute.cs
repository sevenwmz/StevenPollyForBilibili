...
namespace StevenPollyApi.PollyExtend
{
    /// <summary>
    /// 定义AOP特性类及封装Polly策略
    /// </summary>
    ...
    public class PollyPolicyAttribute : Attribute, IInterceptor
    {
        ...
        public void Intercept(IInvocation invocation)
        {
            ...
            {
                Console.WriteLine("Hello,没有特性标注的一个请求   之前");
                // 直接调用方法本身
                invocation.Proceed();
                Console.WriteLine("Hello,没有特性标注的一个请求   之后");
            ...
            else
            {
                ...
                if (pollyPolicyConfigAttribute.CacheTTLMilliseconds > 0)
                {
                    ...
                    string cacheKey = @$"PollyMethodCacheManager_Key_{invocation.Method.DeclaringType}.{invocation.Method}{string.Join("_", invocation.Arguments)}";
                    ...
                    {
                        ExcuteWorkTask(invocation, policy, pollyCtx);
                        //存入缓存中
        ...
        private static void ExcuteWorkTask(IInvocation invocation, AsyncPolicy? policy, Context pollyCtx)
        {
            //如果缓存中没有，则执行实际被拦截的方法
            Task task = policy.ExecuteAsync(async (context) =>
            {
                invocation.Proceed();
